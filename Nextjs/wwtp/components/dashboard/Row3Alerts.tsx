...
export default function Row3Alerts({ latestValues, latestWeather, isDarkMode = true }: AlertProps) {
  ...
  const theme = {
    ...
    itemDanger: isDarkMode ? "bg-red-500/10 border-red-500/30" : "bg-red-50 border-red-100",
    ...
    memoText: isDarkMode ? "text-slate-200" : "text-slate-800",
    memoInput: isDarkMode ? "bg-white/5 border-white/10" : "bg-slate-50 border-slate-200"
  };
  ...
  return (
    <div className={`p-4 rounded-3xl border h-full flex flex-col shadow-inner relative overflow-hidden transition-colors duration-500 ${theme.container}`}>
      ...
      <div className="flex flex-col gap-2 flex-1 overflow-y-auto custom-scrollbar">
        {alerts.map((alert) => (
          <div key={alert.id} className={`px-3 py-2 rounded-xl border transition-all ${alert.status === 'danger' ? theme.itemDanger : theme.itemNormal}`}>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {!alert.details && <span className={`text-[11px] font-mono font-bold ${alert.status === 'danger' ? 'text-red-400' : 'text-slate-400'}`}>{alert.valueText}</span>}
                {alert.status === 'danger' && (
                  <button onClick={() => handlePrepareAction(alert.title, alert.actionContext)} className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-red-500 text-white text-[9px] font-black hover:bg-red-600 shadow-sm transition-transform active:scale-95"><PencilLine className="w-2.5 h-2.5" /> 조치</button>
                )}
              </div>
            </div>
            {alert.details && (
      ...
      {showQuickMemo && (
        <div className={`absolute inset-0 z-20 p-5 flex flex-col justify-center rounded-3xl backdrop-blur-md transition-all ${theme.memoOverlay}`}>
          <div className="flex justify-between items-center mb-3">
            <h4 className={`text-xs font-bold flex items-center gap-2 ${theme.title}`}><PencilLine className="w-3 h-3 text-blue-400" /> 조치 사항 보고</h4>
            <button onClick={() => setShowQuickMemo(false)} className="text-slate-400 hover:text-red-500 transition-colors"><X className="w-4 h-4"/></button>
          </div>
          <textarea 
            className={`w-full h-24 rounded-xl p-3 text-sm outline-none resize-none transition-all focus:ring-1 focus:ring-blue-500 ${theme.memoInput} ${theme.memoText}`}
            value={memoContent} 
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => fileInputRef.current?.click()} className={`flex items-center gap-1 px-2 py-1.5 rounded-lg border text-[10px] font-bold transition-all ${selectedFile ? 'border-blue-500 text-blue-500 bg-blue-50' : 'border-slate-200 text-slate-400 hover:bg-slate-50'}`}>
                {selectedFile ? <ImageIcon className="w-3 h-3" /> : <Paperclip className="w-3 h-3" />} {selectedFile ? '변경' : '첨부'}
              </button>
            </div>
            <button onClick={handleSubmitMemo} disabled={isSubmitting} className="px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-[11px] font-bold flex items-center gap-2 shadow-md transition-all active:scale-95 disabled:opacity-50">
