...
해야할 일 : 
	...
	- 테스트 해보고 시간이 너무 많이 걸리면 데이터를 줄이던지, 방식을 바꿀수 있음
	- 20250501 기준 DB에서 불러온 데이터를 처리하는데 1.19s가 소요됨
	- 20250501 기준 저장된 csv에서 데이터를 불러오는데 0.32s가 소요됨
	- 20250501 기준 DB에서 불러온 데이터를 처리해서 DB로 저장하는데 1.15s로 처리 후 파일로 저장하는 시간과 차이 없음
	- 20250501 기준 DB에 저장된 내용을 불러오는데 0.216s가 소요됨
	...
	!!!!!! 요청을 시간/분으로 받아서 24시간 이전부터의 데이터로 구성
	실시간 데이터 처럼 보이기 위해서는 
	연속하는 날짜의 데이터가 2880..or 2600개 이상인 날짜를 선택되도록
