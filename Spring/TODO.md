...
해야할 일 : 
	예측값의 데이터를 굽이 다른 테이블로 옮기지 말고 쿼리를 통해서 JOIN해서 보여주는 방법을 찾아보기
	SELECT 할때 flow_no or tms_no중 max인 값을 가져와서 보여주면 될것으로 예상되고
	그 외에도 create_time 추가 필요(로그에도 사용될것)
	
	ISR 페이지는 springboot back에서 해주는게 아니고
	nextJS의 서버 기능을 이용해서 미리 구성할수 있도록 처리하는 것임
