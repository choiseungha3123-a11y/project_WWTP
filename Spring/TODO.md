...
해야할 일 : 
	...
	리눅스 업로드시 한글로된 파일명의 이미지가 보이지 않음
	모바일 차트 X축 값을 모두 보여질게 아니고 (시작, 현재, 끝)만 보이는건 어떨지
	모바일 Logout, 다크/라이트모드 버튼을 '개인정보수정' 버튼 아래로 옮기는건 어떨지
	모바일 '시스템,온도,강우'를 타이틀 아래로 옮기는건 어떨지
