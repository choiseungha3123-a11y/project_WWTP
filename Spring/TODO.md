...
해야할 일 : 
	...
	날씨 API 수집할때 -99.9도가 나오는건 제거하도록 처리 필요
	- TA, RN_15m, RN_60m, RN_12H, RN_DAY, HM, TD
	- 예측을 보낼때 -99.9값이 있으면 어떻게 처리할지 유찬님이랑 상의 필요
	...
	기존의 예측값도 실측값과 함께 보여주기
	TN, TP, TOC를 별개의 차트로 
	Legend에서 실선과 점선 구분이 모호함
	시스템 체크는 완리자 아이콘 쪽으로 옮기도록
	다른 수치들이 비정상인거랑 유사성이 없음
	유입유량 등의 4개의 수치와 Event Detection과 유사함
	차트들을 한쪽으로 배치하고
	수치들과 상황 판단을 한쪽으로 배치
	Memo는 별도로 배치하되 작은 공간을 차지하고 필요시 팝업되는 방식도.
	점수가, event detection보다 먼저 보이게 위로 배치
	Memo를 숫자로 보여주고 기존이력도 바로 볼수 있게 밖으로
