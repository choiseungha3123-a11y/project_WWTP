...
from demo.utils.data_loader import get_png_path, load_predictions
...
for selected_target, tab in zip(TARGET_ORDER, tabs):
    with tab:
        ...
        # Semantic color: green=good, amber=ok, red=poor
        r2_accent = SUCCESS if metrics["r2"] >= 0.80 else (WARNING if metrics["r2"] >= 0.65 else DANGER)
        ...
        kpis = [
            ...
            ("MAPE (%)", f"{metrics['mape']:.2f}", PRIMARY),
        ...
        with c1:
            ...
            fig_scatter = px.scatter(
                ...
                y="predicted",
                color="error",
                color_continuous_scale=[[0, ACTUAL_COLOR], [0.5, "#D1D5DB"], [1, PRED_COLOR]],
                opacity=0.65,
        ...
        st.divider()
        st.markdown(section_header("DL 학습 곡선"), unsafe_allow_html=True)
        learning_path = get_png_path("learning_curve", selected_target)
