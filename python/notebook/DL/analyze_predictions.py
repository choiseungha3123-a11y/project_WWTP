...
# 데이터 로드
df = pd.read_csv("C:/project_WWTP/python/results/DL/flow_predictions.csv")
...
low_flow = df[df['actual'] <= low_threshold]
mid_flow = df[(df['actual'] > low_threshold) & (df['actual'] < high_threshold)]
high_flow = df[df['actual'] >= high_threshold]

print(f"낮은 flow (<{low_threshold:.1f}):")
print(f"  샘플 수: {len(low_flow)}")
print(f"  평균 실제값: {low_flow['actual'].mean():.2f}")
print(f"  평균 예측값: {low_flow['predicted'].mean():.2f}")
print(f"  MAE: {low_flow['abs_error'].mean():.2f}")
print(f"  MAPE: {low_flow['pct_error'].mean():.2f}%")

print(f"\n중간 flow ({low_threshold:.1f}-{high_threshold:.1f}):")
print(f"  샘플 수: {len(mid_flow)}")
print(f"  평균 실제값: {mid_flow['actual'].mean():.2f}")
print(f"  평균 예측값: {mid_flow['predicted'].mean():.2f}")
print(f"  MAE: {mid_flow['abs_error'].mean():.2f}")
print(f"  MAPE: {mid_flow['pct_error'].mean():.2f}%")

print(f"\n높은 flow (>{high_threshold:.1f}):")
print(f"  샘플 수: {len(high_flow)}")
print(f"  평균 실제값: {high_flow['actual'].mean():.2f}")
print(f"  평균 예측값: {high_flow['predicted'].mean():.2f}")
print(f"  MAE: {high_flow['abs_error'].mean():.2f}")
print(f"  MAPE: {high_flow['pct_error'].mean():.2f}%")
...
ax2.set_xlabel('실제값 (m³/h)')
ax2.set_ylabel('예측값 (m³/h)')
...
# 4. 구간별 MAPE
ax4 = axes[1, 1]
bins = [df['actual'].min(), low_threshold, high_threshold, df['actual'].max()]
labels = ['낮음', '중간', '높음']
df['flow_category'] = pd.cut(df['actual'], bins=bins, labels=labels)
category_mape = df.groupby('flow_category')['pct_error'].mean()
...
plt.tight_layout()
plt.savefig("C:/project_WWTP/python/results/DL/prediction_analysis.png", dpi=300, bbox_inches='tight')
print(f"\n✓ 분석 플롯 저장: C:/project_WWTP/python/results/DL/prediction_analysis.png")
