...
def add_target_history_features(df, target_cols, lags=None, windows=None, stats=["mean", "std"]):
    """
    target의 과거 정보를 특성으로 추가 (데이터 누수 방지)
    
    예측 대상 변수의 과거 값을 특성으로 사용하되, shift(1)을 적용하여
    ...
    lags : list, optional
        ...
        예: [1, 2, 3] -> 1시간 전, 2시간 전, 3시간 전
    windows : list, optional
        rolling window 시간 리스트 (기본값: [3, 6, 12, 24])
        예: [3, 6] -> 3시간, 6시간 rolling 통계
    stats : list
    ...
    Returns:
    --------
    pd.DataFrame : target 과거 정보 특성이 추가된 데이터프레임
    
    Examples:
    ---------
    >>> df = add_target_history_features(df, target_cols=["FLUX_VU"], lags=[1, 2, 3])
    >>> # FLUX_VU_target_lag1, FLUX_VU_target_lag2, FLUX_VU_target_lag3 생성
    
    >>> df = add_target_history_features(df, target_cols=["TOC_VU", "SS_VU"], 
    ...                                   lags=[1, 2], windows=[3, 6])
    >>> # TOC_VU_target_lag1, TOC_VU_target_lag2, TOC_VU_target_r3_mean, ...
    """
    if lags is None:
        lags = [1, 2, 3, 6, 12, 24]
    if windows is None:
    ...
    for target_col in target_cols:
        ...
        for lag in lags:
            # lag+1을 적용하여 현재 시점 제외
            # 예: lag=1이면 shift(2)로 1시간 전 값 사용
            out[f"{target_col}_target_lag{lag}"] = out[target_col].shift(lag)
        
        # 2) Rolling 통계 특성 (shift(1) 후 계산)
        # 현재 시점을 제외한 과거 window 시간의 통계
        ...
        for w in windows:
            ...
            if "mean" in stats:
                out[f"{target_col}_target_r{w}_mean"] = r.mean()
            
            if "std" in stats:
                out[f"{target_col}_target_r{w}_std"] = r.std()
            
            if "min" in stats:
                out[f"{target_col}_target_r{w}_min"] = r.min()
            
            if "max" in stats:
                out[f"{target_col}_target_r{w}_max"] = r.max()
        
        # 3) 추가 유용한 특성
        # 변화율 (1시간 전 대비)
        out[f"{target_col}_target_delta1"] = out[target_col].diff(1)
        
        # 변화율 (2시간 전 대비)
        out[f"{target_col}_target_delta2"] = out[target_col].diff(2)
        
        # 변화 가속도 (2차 차분)
        out[f"{target_col}_target_accel"] = out[target_col].diff(1).diff(1)
